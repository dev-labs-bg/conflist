...
const InsidePage = (props) => {
    ...
    const eventId = props.location.pathname.substring(7);
    console.log(eventId);
    const events = props.conference.data;
    console.log(events);

    let event;
    props.conference.data.forEach( (ev) => {
        ev.id.indexOf(eventId) ? null : event = { ...ev };
    });
    console.log(event);
    ...
    return (
        <div className="container__register mx-auto pt-5 pb-5 d-flex flex-column">
            ...
            <div className="mb-1">
                ...
                <span className="card__dates mb-2">{getFormattedDate(event.start, event.end)}
                ...
                <div className="mb-4 text-bottom d-flex">
                    ...
                    <h5 className="font-weight-normal d-inline">Going: <span className="text-secondary">{event.atendees}</span></h5>
                </div>
                {event.description}
...
export default connect(mapStateToProps)(InsidePage);
